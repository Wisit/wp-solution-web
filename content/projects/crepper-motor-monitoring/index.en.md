...
### System Workflow
1.  **Data Acquisition:** An Industrial PC connects to 3 Digital Meters via **RS-485 (Modbus RTU)**.
2.  **Processing (Node-RED):** Node-RED polls current, voltage, and power data every second, processing and filtering signal noise.
3.  **Storage (MySQL):** Processed data is logged into a **MySQL Database (V8.x)** for long-term historical analysis.
4.  **Visualization (Vue.js):** A responsive web dashboard displays real-time gauges and historical line charts, allowing engineers to track motor health instantly.
